...
function features = GetWindowFeatureVectors(im, x, y, windowWidth, windowHeight)

    columnPixels = idivide(windowWidth, 2);
    rowPixels = idivide(windowHeight, 2);
    
    patch = im(y-rowPixels:y+rowPixels, x-columnPixels:x+columnPixels);
    [R, C] = size(patch);
